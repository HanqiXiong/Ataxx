...
class GUI extends TopLevel implements View, CommandSource, Reporter {
    ...
    private synchronized void newGame(String unused) {
        ...
        select("Options->Players->Red AI", false);
        select("Options->Players->Blue Manual", false);
        select("Options->Players->Red Manual", true);
        select("Options->Players->Blue AI", true);
    }
    ...
    public void announceWin(PieceColor player) {
        if (player == RED || player == BLUE) {
            showMessage("" + player + " wins", "Outcome", "information");
        } else if (player == EMPTY) {
            showMessage("Draw", "Outcome", "information");
        }
        send("manual red");
        send("auto blue");
    }
